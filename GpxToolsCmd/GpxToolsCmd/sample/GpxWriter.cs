...
namespace Gpx
{
    public class GpxWriter : IDisposable
    {
        ...
        public void WriteMetadata(GpxMetadata metadata)
        {
            ...
            if (!string.IsNullOrEmpty(metadata.Name)) Writer_.WriteElementString("name", metadata.Name);
            if (!string.(metadata.Description)) Writer_.WriteElementString("desc", metadata.Description);
